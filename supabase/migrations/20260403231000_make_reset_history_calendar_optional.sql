...
BEGIN
  ...
  IF to_regclass('public.calendar_sync_jobs') IS NOT NULL THEN
    EXECUTE 'DELETE FROM public.calendar_sync_jobs WHERE user_id = $1'
    USING v_user_id;
  END IF;
  ...
  IF to_regclass('public.calendar_connections') IS NOT NULL THEN
    EXECUTE 'UPDATE public.calendar_connections SET last_sync_error = NULL WHERE user_id = $1'
    USING v_user_id;
  END IF;
