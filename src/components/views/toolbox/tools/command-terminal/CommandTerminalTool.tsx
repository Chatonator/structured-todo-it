...
import { ExecutionLog, LogLevel, StoredScript } from './terminalTypes';
import {
  ExecutionLogCard,
  GuideCard,
  ScriptLibraryCard,
  ScriptSummaryCard,
  TerminalComposerCard,
} from './CommandTerminalPanels';
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.9fr)]">
      <TerminalComposerCard
        ...
        onScriptLabelChange={setScriptLabel}
        onExample={() => setScript(DEFAULT_SCRIPT)}
        onClear={() => setScript('')}
        onFavorite={() => saveCurrentScript('favorite')}
        onToggleJson={() => setJsonMode(value => !value)}
      ...
      <div className="space-y-6">
        <ScriptSummaryCard parsePreview={parsePreview} executionPlan={executionPlan} />
        <ScriptLibraryCard
        ...
        <GuideCard
          ...
          rules={COMMAND_RULES}
          externalAiPrompt={EXTERNAL_AI_PROMPT}
          onCopyPrompt={copyExternalAiPrompt}
        />
        <ExecutionLogCard logs={logs} styles={LOG_STYLES} labels={LOG_LABELS} />
      </div>
    </div>
  );
};
