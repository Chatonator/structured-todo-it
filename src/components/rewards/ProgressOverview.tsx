...
import { STREAK_MIN_IMPORTANT_MINUTES, TIME_TIERS, GAUGE_MAX_MINUTES } from '@/lib/rewards/constants';
...
const ProgressOverview: React.FC<ProgressOverviewProps> = ({
  ...
  const minutesAvailable = progress.minutesAvailable ?? 0;
  const fillPct = Math.min(100, Math.round((minutesAvailable / GAUGE_MAX_MINUTES) * 100));
  ...
  return (
    <Card className="p-3 border-primary/20 h-full w-full min-w-[160px] flex flex-col">
      ...
      <div className="flex items-center gap-1.5 mb-2 shrink-0">
        ...
        <h3 className="text-sm font-bold text-foreground">Temps libre</h3>
      </div>
      ...
      <div className="flex gap-4 flex-1 min-h-0">
        ...
        <div className="relative shrink-0 flex">
          <div className="relative w-7 bg-muted rounded-full overflow-hidden border border-border flex-1">
            ...
            />
            {TIME_TIERS.map(t => {
              const pos = Math.round((t / GAUGE_MAX_MINUTES) * 100);
              return (
                <div
                  key={t}
          ...
          {/* Tier labels — aligned to gauge height */}
          <div className="absolute left-9 top-0 bottom-0 flex flex-col-reverse justify-between py-0.5">
            {TIME_TIERS.map(t => (
              <span
                key={t}
                className={`text-[10px] leading-none whitespace-nowrap ${minutesAvailable >= t ? 'text-primary font-bold' : 'text-muted-foreground'}`}
              >
                {t}
              </span>
            ))}
        ...
        <div className="flex flex-col justify-between min-w-0 flex-1 pl-4 py-0.5">
          <div>
            <span className="text-3xl font-bold text-foreground leading-none">{minutesAvailable}</span>
            <p className="text-xs text-muted-foreground mt-0.5">min disponibles</p>
          </div>
          ...
          <p className="text-[10px] text-muted-foreground leading-tight">
            Gagné {progress.totalMinutesEarned ?? 0} · Dépensé {progress.totalMinutesSpent ?? 0} min
          </p>
        </div>
      </div>
    </Card>
