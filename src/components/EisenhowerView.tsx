  ...
  const quadrants = React.useMemo(
    () => ({
      'urgent-important': {
        ...
        tasks: tasks.filter(
          (task) => getCategoryQuadrant(task.category) === 'urgent-important'
        )
      },
      'important-not-urgent': {
        ...
        tasks: tasks.filter(
          (task) => getCategoryQuadrant(task.category) === 'important-not-urgent'
        )
      },
      'urgent-not-important': {
        ...
        tasks: tasks.filter(
          (task) => getCategoryQuadrant(task.category) === 'urgent-not-important'
        )
      },
      'not-urgent-not-important': {
        ...
        tasks: tasks.filter(
          (task) => getCategoryQuadrant(task.category) === 'not-urgent-not-important'
        )
      }
    }),
    [tasks]
      ...
      >
        ...
        <div className="flex items-center space-x-2 text-xs text-muted-foreground">
          ...
          {task.isCompleted && (
            <Badge
              variant="outline"
              className="text-xs bg-system-success/10 text-system-success"
            >
              Terminée
            </Badge>
          )}
    ...
    return (
      <div className="text-center py-8">
        ...
        <p className="text-sm text-muted-foreground">
          Créez des tâches pour voir la matrice d'Eisenhower
        </p>
      </div>
    );
  }
            ...
            return (
              ...
              >
                ...
                <div className="text-xs text-muted-foreground">
                  {formatDuration(getTotalTime(quadrant.tasks))}
                </div>
              </div>
            );
          })}
                ...
                {quadrant.tasks.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-border">
                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                      <span>
                        Total: {quadrant.tasks.length} tâche
                        {quadrant.tasks.length > 1 ? 's' : ''}
                      </span>
      ...
      <Card className="bg-accent border-border">
        ...
        <CardContent className="text-sm text-foreground">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <h4 className="font-semibold mb-1 text-category-obligation">
                🔥 Urgent & Important
              </h4>
              <p className="text-xs text-muted-foreground">
                Traitez immédiatement ces tâches. Elles ne peuvent pas attendre.
              </p>
            </div>
            <div>
              <h4 className="font-semibold mb-1 text-category-envie">
                🎯 Important & Non Urgent
              </h4>
              <p className="text-xs text-muted-foreground">
                Planifiez du temps dédié. C'est ici que vous devez investir le
                plus.
              </p>
            </div>
            <div>
              <h4 className="font-semibold mb-1 text-category-quotidien">
                ⚡ Urgent & Non Important
              </h4>
              <p className="text-xs text-muted-foreground">
                Déléguez si possible, ou traitez rapidement.
              </p>
            </div>
            <div>
              <h4 className="font-semibold mb-1 text-category-autres">
                🗑️ Non Urgent & Non Important
              </h4>
              <p className="text-xs text-muted-foreground">
                Éliminez ou minimisez ces activités.
              </p>
            </div>
          </div>
        </CardContent>
